...
export class LoginComponent implements OnInit {
    ...
    constructor(private authSocialService: SocialAuthService,public layoutService: LayoutService,private messageService: MessageService,private authService:AuthService ,private router: Router,public httpClient: HttpClient, public secureStorageService : SecureStorageService ) { }
    ...
    ngOnInit(): void {
        google.accounts.id.initialize({
            ...
            error => {
                this.isError = true;
                this.messageService.add({severity:"error", summary:"Error when login with Google"});
                // console.log(error)
            }
     ...
     async login() {
       this.loading = true;
       await this.httpClient.post<any>('/api/v1/project/auth/signin',{'userName':this.username,'passWord':this.password}).toPromise().then(
           data => {
            if(data.resultCode == "0")
              {
                this.authService.setToken(data.data);
    ...
    async loginWithGoogle() {
        ...
        await this.authSocialService.signIn(GoogleLoginProvider.PROVIDER_ID).then(
            data => {
                console.log(data)
            }
        );
      ...
      async loginWithFacebook() {
        ...
        await this.authSocialService.signIn(FacebookLoginProvider.PROVIDER_ID).then(
            data => {
        ...
        if(!this.isError) {
            this.loading = true;
            await this.httpClient.post<any>('/api/v1/project/auth/loginFacebook',{'id':this.socialUser.id,'authToken':this.socialUser.authToken,'name':this.socialUser.name,'email':this.socialUser.email}).toPromise().then(
                data => {
                 if(data.resultCode == "0")
                   {
                     this.authService.setToken(data.data);
                 ...
                 error => {
                    this.messageService.add({severity:"error", summary:"Error when login with facebook"});
